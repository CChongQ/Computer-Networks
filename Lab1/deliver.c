...
int main(int argc, char const *argv[]){
    ...
    while (1){
        ...
        unsigned int size_temp = fread(filedata_buf,1,MAX_LEN,file_tb_transfer);

        // temp_packet.size = size_temp;
        printf("size_temp 1 = %d !\n",size_temp);
        
        // memcpy(temp_packet.filedata, filedata_buf, size_temp);

        //Format buf
        printf("HERE 1!\n");
 
        char packet_buf[MAX_LEN];
        // printf("c.total_frag= %d !\n",temp_packet.total_frag);
        // printf("temp_packet.frag_no= %d !\n",temp_packet.frag_no);
        // printf("temp_packet.filename = %s !\n",temp_packet.filename);
        // int written_size = sprintf(packet_buf, "%d:%d:%d:%s:", temp_packet.total_frag, temp_packet.frag_no, size_temp , temp_packet.filename);
        // printf("num_frag= %d !\n",num_frag);
        // printf("i+1= %d !\n",i+1);
        // printf("size_temp= %d !\n",size_temp);
        // printf("fileName= %s !\n",fileName);
        int written_size = sprintf(packet_buf, "%d:%d:%d:%s:", num_frag, i+1, size_temp , fileName);
        printf("packet_buf send = %s \n",packet_buf);
